...
with open("../TData/sbx_Servo.json", "r", encoding="utf-8") as file:
    data = json.load(file)
logging.info("JSON 文件加载成功。")  # 在加载成功后打印消息

# 按照键的顺序处理JSON数据
for key in sorted(data.keys()):
    if key.startswith("sbx"):
        # 获取当前键对应的值
        ...
        # 指定原始图片文件路径
        original_image_path = "..\\templet\\sbx.jpg"
